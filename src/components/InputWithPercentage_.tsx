...
export default function InputWithPercentage({
  ...
  }, [assetSelected, subTabSelected, depositValue]);
  ...
  const updateToValuePercentage = (value: number) => {
    ...
    if (!disabled) {
      ...
      else inputVal = maxValue * value / 100
    ...
    <div className='my-[20px]'>
      <FormControl variant="outlined">
        <OutlinedInput
          ...
          value={curValue >= 0 ? (subTabSelected === "NFTs" || subTabSelected === "Merge" || subTabSelected === "BurnNFT" ? Math.floor(curValue) : curValue) : 0}
          onChange={(event) => {
